...
public class Application implements ClientSocketListener {
	...
	private KVClient client = null;
        private KVServer sv = null;
        private  KVStore mystore = null;
		...
		if(tokens[0].equals("quit")) {	
			...
			if(tokens.length >= 2) {
				...
				printError("No message passed!");
					...
					printError("Could not put value!");
					logger.warn("Could not put value!", e);
				}
					...
					printError("Could not get value!");
	...
	private void sendMessage(String msg){
		...
		try {
			 mystore .put(key, value);
                            KVServer.logger.log(Level.INFO, "SUCCESS: <" + key + ","+value +">");
                        
                         		} catch (Exception e) {
                                             KVServer.logger.log(Level.INFO, "FAILED: <" + key + ","+value +">");
			printError("Unable to put value!");
		...
		try {
			KVBasicMessage value = sv.get(key);
                         if (value.getValue() != null) {
                              KVServer.logger.log(Level.INFO, "VALUE: "+ value.getValue());
                         }
                         else {
                              KVServer.logger.log(Level.INFO, "NO VALUE FOUND FOR KEY: "+ key);
                         }
                } catch (Exception e) {
			printError("Unable to put value!");
	...
	private void connect(String address, int port) 
        ...
        try {
            mystore = new KVStore("localhost", 55555);
